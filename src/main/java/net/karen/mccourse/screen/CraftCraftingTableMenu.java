...
public class CraftCraftingTableMenu extends RecipeBookMenu<CraftingContainer> {
    ...
    protected static void slotChangedCraftingGrid(AbstractContainerMenu pMenu, Level pLevel, Player pPlayer, CraftingContainer pContainer, ResultContainer pResult) {
        if (!pLevel.isClientSide) {
            ...
            Optional<CraftingRecipe> optional = Objects.requireNonNull(pLevel.getServer()).getRecipeManager()
                    .getRecipeFor(RecipeType.CRAFTING, pContainer, pLevel);
    ...
    @Override
    public boolean stillValid(@NotNull Player pPlayer) { return stillValid(this.access, pPlayer, ModBlocks.CRAFT_CRAFTING_TABLE.get()); }
