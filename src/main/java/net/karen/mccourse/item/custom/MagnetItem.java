...
public class MagnetItem extends Item {
    // Radius area to take items on Player's inventory
    ...
    private void messageScreen(Player player, String name, ChatFormatting color) {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level level, Player player,
        ...
        if (!player.level().isClientSide() && stack.getItem() instanceof MagnetItem) {
            ...
            // Magnet active message on SCREEN
            if (activeMagnet(stack)) {
                messageScreen(player, "ACTIVE", ChatFormatting.DARK_GREEN);
            }
            // Magnet disable message on SCREEN
            if (!activeMagnet(stack)) {
                messageScreen(player, "DISABLE", ChatFormatting.DARK_RED);
            }
        }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, @NotNull
        ...
        // Call active tag
        boolean compoundTag = stack.getOrCreateTag().getBoolean("active");

        // Magnet active message on TOOLTIP
        if (compoundTag) {
            message(tooltip, "Activated", ChatFormatting.DARK_GREEN);
        }

        // Magnet disable message on TOOLTIP
        if (!compoundTag) {
            message(tooltip, "Inactivated", ChatFormatting.DARK_RED);
        }
    }

    // Magnet message depends of active CUSTOM TAG
    private void message(List<Component> tooltip, String name, ChatFormatting color) {
        tooltip.add(Component.literal(name + " Magnet")
                .setStyle(Style.EMPTY.applyFormats(color, ChatFormatting.BOLD)));
    }
}