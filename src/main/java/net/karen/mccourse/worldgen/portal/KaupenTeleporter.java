...
public class KaupenTeleporter implements ITeleporter {
    ...
    public KaupenTeleporter(BlockPos pos, boolean insideDim) { thisPos = pos; insideDimension = insideDim; }
    ...
    public Entity placeEntity(Entity entity, ServerLevel currentWorld, ServerLevel destinationWorld,
        ...
        if (!insideDimension) { y = thisPos.getY(); }
        ...
        if (insideDimension) {
            ...
            for (BlockPos checkPos : BlockPos.betweenClosed(destinationPos.below(10).west(10),
                ...
                if (destinationWorld.getBlockState(checkPos).getBlock() instanceof KaupenPortalBlock) { doSetBlock = false; break; }
            }

            if (doSetBlock) { destinationWorld.setBlock(destinationPos, ModBlocks.KAUPEN_PORTAL.get().defaultBlockState(), 3); }
        }
        return entity;
    }
}