...
public class FlyEffect extends MobEffect {
    ...
    public void applyEffectTick(@NotNull LivingEntity entity, int amplifier) {
        fly(entity, 0.05f + (0.02f * amplifier), 0.1f + (0.02f * amplifier));
    ...
    private static void fly(LivingEntity entity, float fly, float walk) {
        if (entity instanceof Player player && !player.level().isClientSide()) {
            Abilities abilities = player.getAbilities();
            ModEvents.flyEffect(new TickEvent.PlayerTickEvent(TickEvent.Phase.END, player));
            abilities.setFlyingSpeed(fly);
            abilities.setWalkingSpeed(walk);
        }
    }
}